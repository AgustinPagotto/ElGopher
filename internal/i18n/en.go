package i18n

var EN = map[string]string{
	"blog":            "Blog",
	"about":           "About",
	"nav.home":        "Home",
	"nav.projects":    "Projects",
	"nav.logout":      "Logout",
	"articles.header": "El Gopher Articles",
	"home.header":     "Welcome",
	"home.whoami":     "Who am I?",
	"home.whoamiparagraph": `Hello! I'm Agustin!
I work remotely as a Software Engineer at Teladoc Health. Previously, I worked at Tsoft for 2 years.
I have a barchelors degree en System Engineering and I live in a small city in the north of Argentina.`,
	"home.blogparagraph":       `This website has the purpose of introducing myself, and also to be a place to write about technical and not so technical stuff about Go.`,
	"home.gotolastarticlelink": "Go to my latest article -> ",
	"home.contact":             "Contact",
	"home.contactparagraph":    "If you have a doubt about the site, have a job oportunity that i might be interested in or you just want to talk about programming or go, don't hesitate to reach out!",
	"about.techstack":          "ElGopher Page Tech Stack",
	"about.gotechstack":        " for backend logic and templating",
	"about.htmxtechstack":      " for lightweight interactions",
	"about.psqltechstack":      " for persistence",
	"about.picotechstack":      " for lightweight styling",
	"about.thanks":             "Thanks!",
	"about.thanksparagraph": ` This section will be just to give thanks to all the authors/people that let me learn from his books, podcasts, articles, etc. and made my journey to being a Go developer possible.
	Thanks a lot to:`,
}
